...
/*
    * 단, DML명령어로 조작이 불가능한 경우가 더 많음
    ...
    2) 뷰에 정의되어있는 컬럼 중에 베이스테이블 상에 NOT NULL제약조건이 지정되어있는 경우
...
DELETE
  FROM VM_JOB
 WHERE JOB_NAME = '사원';
 -- 외래키가 되어있는 경우 자식테이블에서 사용하고 있으면 삭제 안됨
...
SELECT * FROM VM_GROUPDEPT;
SELECT * FROM EMPLOYEE;

--INSERT (오류)
INSERT INTO VM_GROUPDEPT VALUES('D3',20000000,4000000);
...
--INSERT(오류)8+
INSERT INTO VM_JOIN VALUES(700,'황장우','총무부');
...
/*
   *VIEW\ DHQTUS
   ...
   2)FROCE | NOFORCE
     > FROCE : 서브쿼리에 기술된 테이블이 존재하지 않아도 뷰가 생성됨
     > NOFORCE : 서브쿼리에 기술된 테이블이 존재해야만 뷰를 생성할 수 있음 (생략시 기본값)
   3)WITH CHECK OPTION : DML시 서브 쿼리에 기술된 조건에 부합하는 값으로만 DML이 가능하도록 함
...
-- TT테이블을 생성해야만 그때부터 VIEW를 활용할 수 있음
CREATE TABLE TT(
    TCODE NUMBER, 
    TNAME VARCHAR2(20),
    TCOUNT NUMBER
...
DELETE FROM VM_EMP_READ WHERE EMP_ID = 200;--DHFB

...
SELECT EMP_ID, EMP_NAME, SUBSTR(EMP_NO, 8, 1) 성별
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO, 8, 1) IN (1,3); -- emp_no의 8번째부터 1개가  1이나 3이면 가져옴
...
/*
    ...
    CONCAT('문자열', '문자열')
*/
SELECT CONCAT('oracle','(오라클)') from dual;  --콘캣은 여러개를 연결 가능하지 않고 딱 2개만 가능하다
...
/*
    NVL2(컬럼, 반환값 1, 반환값 2)
    -컬럼값이 존재할 경우 반환값은 1
    -컬럼값이 NULL일 경우 반환값은 2
    
*/
--보너스를 받는 사람은 50%의 성과급을 , 보너스를 받지 않는 사람은 10%의 성과급을 조회
...
/*
 NULLIF(비교대상1, 비교대상2)
 - 두개의 값이 일치하면 NULL반환
 - 두개의 값이 일치하지 않으면 비고대상1을 반환(앞의 값을 EX/ '1234', '4567' 이면 '1234'를 반환
 */
...
*/
--사원명 , 성별
SELECT EMP_NAME,DECODE (SUBSTR(EMP_NO ,8,1), '1', '남자', '2','여자','혼합')성별
FROM EMPLOYEE;
...
--  DECODE(JOB_CODE, 'J7', SALARY * 1.10, <= 이게 더 좋아보임
...
------------------------------------------------------------------------------
/*
 MIN(모든 타입컬럼): 해당 컬럼값들 중에 가장 작은 값 반환
 MAX(모든 타입컬럼): 해당 컬럼값 들 중에 가장 큰 값 반환
*/
--사전순으로 이름이 가장 작은값, 급여중 가장 적게 받는 값, 입사일중 가장 먼저 입사한 날짜
SELECT MIN(EMP_NAME), MIN(SALARY),MIN(HIRE_DATE) -- 단일행 함수와,그룹함수를 함께 쓸 수 없다
FROM EMPLOYEE;
--사전순으로 이름이 가장 큰값, 급여중 가장 크게 받는 값, 입사일중 가장 나중에 입사한 날짜
